/**
 * import-champions-weekly-2025-mca.ts
 * -------------------------------------
 * Reads data/outputs/champions_weekly_2025_mca.json and populates MCA data
 * on the Sunday Service Report for Champions locations (Apr–Jul 2025).
 *
 * Idempotent: skips (group, reportingPeriod) that already have an mca row.
 *
 * Run:
 *   npm run import:champions:2025:mca
 */
...
const DATA_FILE = path.join(
  __dirname,
  '../../data/outputs/champions_weekly_2025_mca.json',
);
...
async function run() {
  if (!fs.existsSync(DATA_FILE)) {
    ...
    Logger.error(
      'Run: python3 data/scripts/extract_champions_weekly_2025.py first',
    );
    process.exit(1);
  }
  ...
  Logger.log(
    `Loaded ${raw.recordCount} MCA records | ${raw.locationCount} locations | ${raw.weekCount} weeks`,
  );
  ...
  if (!report) {
    Logger.error('Sunday Service Report not found.');
    await app.close();
    process.exit(1);
  }
  ...
  if (!mcaField || !locationNameField || !locationIdField) {
    Logger.error('Required fields missing on Sunday Service Report.');
    await app.close();
    process.exit(1);
  }
  ...
  const existingMcaKeys = new Set(
    existingMca.map((r) => `${r.groupId}::${r.reportingPeriod}`),
  );
  Logger.log(`${existingMcaKeys.size} existing mca rows — will skip`);
  ...
  const existingSubmissions: Array<{
    ...
    `SELECT s.id, s."groupId", TO_CHAR(s."reportingPeriod", 'YYYY-MM-DD') AS "reportingPeriod"
       FROM report_submission s WHERE s."reportId" = $1 AND s."reportingPeriod" IS NOT NULL`,
    [report.id],
  );
  ...
  for (const rec of raw.records) {
    ...
    if (!submissionId) {
      const sub = await submissionRepo.save(
  ...
  Logger.log(`New submissions      : ${created}`);
  Logger.log(`Gap-filled           : ${gapFilled}`);
  Logger.log(`Skipped (has mca)    : ${skipped}`);
  Logger.log(`No group match       : ${noGroup}`);
  Logger.log('✅ Champions 2025 MCA import complete');
