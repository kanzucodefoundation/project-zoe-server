 ...
 *
 * Idempotent: skips (group, reportingPeriod) that already have a grade row.
 *
 * Run:
...
async function run() {
  ...
  // Index existing grade rows for idempotency
  const existingGrade: Array<{ groupId: number; reportingPeriod: string }> =
    await conn.query(
     ...
     WHERE rs."reportId" = $1 AND rf.name = 'grade'`,
      [report.id],
    );
  const existingKeys = new Set(
    existingGrade.map((r) => `${r.groupId}::${r.reportingPeriod}`),
  );
  Logger.log(`${existingKeys.size} existing grade rows — will skip`);
