/**
 * import-champions-weekly-2025-pga.ts
 * -------------------------------------
 * Reads data/outputs/champions_weekly_2025_pga.json and populates PGA data
 * on the Sunday Service Report for Champions locations (Jan–Feb 2025).
 *
 * Idempotent: skips (group, reportingPeriod) that already have a pga row.
 *
 * Run:
 *   npm run import:champions:2025:pga
 */
...
const DATA_FILE = path.join(
  __dirname,
  '../../data/outputs/champions_weekly_2025_pga.json',
);
...
async function run() {
  ...
  Logger.log(
    `Loaded ${raw.recordCount} PGA records | ${raw.locationCount} locations | ${raw.weekCount} weeks`,
  );
  ...
  const existingPgaKeys = new Set(
    existingPga.map((r) => `${r.groupId}::${r.reportingPeriod}`),
  );
  Logger.log(`${existingPgaKeys.size} existing pga rows — will skip`);
  ...
  Logger.log('✅ Champions 2025 PGA import complete');
