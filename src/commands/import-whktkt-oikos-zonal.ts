/**
 * import-whktkt-oikos-zonal.ts
 * --------------------------------
 * Reads data/outputs/whktkt_oikos_zonal.json (produced by
 * extract_whktkt_oikos_zonal.py) and populates weekly Oikos metrics
 * (mcs, mcm, mca, salvations, visitations) on the Weekly Oikos Report
 * for each WHKTKT Zone group.
 *
 * Strategy:
 *   - If a submission already exists for (report, group, reportingPeriod)
 *     and already has an 'mcs' row, skip it.
 *   - If no submission exists, create one with all available (non-null) fields.
 *
 * Idempotent: skips any (group, reportingPeriod) that already has an mcs row.
 *
 * Run:
 *   npm run import:whktkt:oikos
 */
...
async function run() {
  if (!fs.existsSync(DATA_FILE)) {
    ...
    Logger.error(
      'Run: python3 data/scripts/extract_whktkt_oikos_zonal.py first',
    );
    process.exit(1);
  }
  ...
  Logger.log(
    `Loaded ${raw.recordCount} records | ${raw.zoneCount} zones | ${raw.weekCount} weeks`,
  );
  ...
  // WHKTKT location → its Zone children
  const whktkt = await groupRepo
    .createQueryBuilder('g')
    .where('g."tenantId" = :tenantId', { tenantId: tenant.id })
    .andWhere("g.\"metaData\"->>'code' = 'WHKTKT'")
    .getOne();
  if (!whktkt) {
    Logger.error('WHKTKT location not found. Run seed:whm:groups first.');
    await app.close();
    process.exit(1);
  }

  const zones = await groupRepo
    .createQueryBuilder('g')
    .where('g."tenantId" = :tenantId', { tenantId: tenant.id })
    .andWhere('g."parentId" = :parentId', { parentId: whktkt.id })
    .getMany();

  const zoneByName = new Map<string, Group>();
  for (const zone of zones) {
    zoneByName.set(zone.name, zone);
  }
  Logger.log(`Found ${zoneByName.size} WHKTKT zones`);
  ...
  for (const rec of raw.records) {
    const group = zoneByName.get(rec.zone);
    if (!group) {
      noGroup++;
      continue;
    }
  ...
  Logger.log('✅ WHKTKT zonal Oikos import complete');
